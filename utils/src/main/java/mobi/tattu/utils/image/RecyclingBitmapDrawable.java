...
public class RecyclingBitmapDrawable extends BitmapDrawable {
	...
	/**
	 * Notify the drawable that the displayed state has changed. Internally a count is kept so that the drawable knows when it is no longer
	 * being displayed.
	 * 
	 * @param isDisplayed
	...
	/**
	 * Notify the drawable that the cache state has changed. Internally a count is kept so that the drawable knows when it is no longer
	 * being cached.
	 * 
	 * @param isCached
	...
	private synchronized void checkState() {
		...
		if (mCacheRefCount <= 0 && mDisplayRefCount <= 0 && mHasBeenDisplayed
				&& hasValidBitmap()) {
			if (Utils.isDebug()) Logger.d(this, "No longer being used or cached so recycling. " + toString());
